...
- **QR 코드 생성** — URL(또는 텍스트) **자체를 코드에 담아 만료가 없음** — 리다이렉트·서버·로그인 없이 브라우저 안에서만 생성되고 URL·로고는 어디로도 전송되지 않음. utm_source/medium/campaign/content/term 전부 지원 + 페이스북·뉴스레터·인쇄물·유튜브 프리셋, 로고 삽입(오류 정정 자동 H), 색 변경(대비 경고), 오류 정정 L/M/Q/H, PNG 300~1200px·SVG 다운로드. 인코더는 MIT 라이선스 qrcode-generator 번들(vendor/qrcode.js, 지연 로드)
