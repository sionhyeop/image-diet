...
- **PDF 탭 (양방향)** — 이미지 여러 장 → PDF 한 파일(자체 PDF 라이터), 반대로 **PDF → 페이지별 이미지**(PNG/JPEG/WebP, 해상도 선택)도 같은 탭에서 토글로 전환. PDF 렌더링은 번들된 pdf.js(1.5MB, 첫 사용 시 1회 로드 후 오프라인)
- **GIF 탭 (양방향)** — 이미지 여러 장을 순서·전환 간격을 정해 움직이는 GIF로(트레이 드래그 순서 변경), 영상은 **크롭(프레임 좁혀나가기) + 타임라인 트림(자동 확대 미세 조정)**으로 원하는 구간만. 재생 속도(0.5~4×)·fps(5~20)·반복(무한/1회) 조절, **만들기 전 길이·용량 예측**, **출력 3MB 자동 맞춤**. GIF89a 인코더(median-cut 팔레트 + LZW + 프레임 차분 압축) 자체 구현, 의존성 0
