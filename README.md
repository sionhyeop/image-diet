...
- **Base64 디코더 (만능)** — 문자열을 원래 파일로 복원. 이미지뿐 아니라 **PDF(첫 쪽 미리보기)·오디오/영상(바로 재생)·텍스트/JSON(내용 미리보기)·ZIP·글꼴** 등 매직 바이트로 형식 자동 감지, 애매하면 형식 칩으로 직접 지정. **Word·Excel·PowerPoint·한글(HWP/HWPX) 문서는 컨테이너 내부까지 확인해 올바른 확장자(.docx 등)로 저장**. 모르는 데이터도 .bin으로 저장. 반대로 **아무 파일 → Base64**(Data URI) 인코딩도 지원(20MB 이하)
