/*! qr v0.7.0 decode — https://github.com/paulmillr/qr
 * Copyright (c) 2023 Paul Miller (paulmillr.com)
 * Dual-licensed (MIT OR Apache-2.0) — MIT 라이선스로 사용.
 * esbuild IIFE 번들: window.QRDecodeLib.decodeQR({width,height,data})
 */
(() => {
  // qrpkg/package/index.js
  ...
  var Payload = {
    create(capacity) {
      ...
      const views = new Array(capacity + 1);
  ...
  var validateImage = (img, named, layout, capacity) => {
    const px = validateSize(img, '"img"');
    if (capacity && (img.width > capacity.width || img.height > capacity.height))
      throw new RangeError(`"img" expected dimensions <= ${capacity.width}x${capacity.height}, got ${img.width}x${img.height}`);
    const data = img.data;
  ...
  var _QRScanner = class {
      ...
      for (let sy = -3; sy <= 3; sy++)
        for (let sx = -3; sx <= 3; sx++) {
          ...
          for (let y = -2; y <= 2 && errors < cap2; y++)
            for (let x = -2; x <= 2; x++) {
              const dark = !!this.read(s, map, scale + (bx + dx + x) / side, scale + (by + dy + y) / side);
              const edge = Math.abs(x) === 2 || Math.abs(y) === 2;
              if (dark !== (edge || x === 0 && y === 0) && ++errors >= cap2)
                break;
            }
    ...
    retain(index, rank, count, capacity, offset = 0) {
    ...
    findBasicAlign(layer, ex, ey, ms) {
      ...
      for (let y = yLo; y <= yHi; y++) {
        let x = xLo;
        if (bit(layer, x - 1, y) === dark)
          x += run(layer, x, y, 1, 0, dark, Infinity);
        while (x <= xHi) {
  ...
  // qr-entry.mjs
  window.QRDecodeLib = { decodeQR: decode_default };
})();
/*!
 * Copyright (c) 2023 Paul Miller (paulmillr.com)
